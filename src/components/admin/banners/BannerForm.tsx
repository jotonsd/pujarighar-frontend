...
import { Banner, useCreateBannerMutation, useUpdateBannerMutation } from '@/api/banners/bannersApi'
...
export default function BannerForm({ editItem, onClose }: Props) {
  ...
  const [form, setForm] = useState(editItem ? {
    title_bn: editItem.title_bn, title_en: editItem.title_en,
    subtitle_bn: editItem.subtitle_bn, subtitle_en: editItem.subtitle_en,
    badge_text: editItem.badge_text, bg_color: editItem.bg_color,
    link: editItem.link, order: editItem.order,
  } : { ...EMPTY_FORM })
  ...
  return (
    <div className="card mb-6 space-y-4">
      <h2 className="font-semibold text-gray-700">
        {editItem ? (locale === 'bn' ? 'ব্যানার সম্পাদনা' : 'Edit Banner') : (locale === 'bn' ? 'নতুন ব্যানার' : 'New Banner')}
      </h2>
      ...
      <div className="grid grid-cols-3 gap-4">
        <FloatingInput label={locale === 'bn' ? 'ব্যাজ টেক্সট' : 'Badge text'} value={form.badge_text} onChange={f('badge_text')} />
      ...
      <div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {BG_PRESETS.map(color => (
            <button key={color} type="button" onClick={() => setForm(p => ({ ...p, bg_color: color }))}
          ...
          <input type="color" value={form.bg_color}
            ...
            className="w-8 h-8 rounded-full border border-gray-200 cursor-pointer p-0.5" title="Custom color" />
      ...
      <div>
        ...
        <div className="flex items-center gap-3">
          ...
          <input ref={fileRef} type="file" accept="image/*" className="hidden"
            onChange={e => { const file = e.target.files?.[0]; if (!file) return; setImageFile(file); setImagePreview(URL.createObjectURL(file)) }} />
        </div>
      </div>

      <div className="flex gap-3">
        <button onClick={handleSave} disabled={saving} className="btn-primary">
          {saving ? '...' : editItem ? (locale === 'bn' ? 'সংরক্ষণ করুন' : 'Save Changes') : (locale === 'bn' ? 'তৈরি করুন' : 'Create')}
        </button>
        <button onClick={onClose} className="btn-secondary">{locale === 'bn' ? 'বাতিল' : 'Cancel'}</button>
      </div>
    </div>
  )
}
