...
export default function PackageList() {
  ...
  const { data, isLoading } = useGetProductsQuery({ page, is_package: 'true', include_inactive: true })
  ...
  const discountLabel = (p: Product) => {
    ...
    if (p.discount_type === 'FLAT') return formatAmount(p.discount_value, locale, 0)
    return '—'
  }
  ...
  const columns: Column<Product>[] = [
    {
      ...
      accessor: p => <div><p className="font-medium text-gray-800">{locale === 'bn' ? p.name_bn : p.name_en}</p><p className="text-xs text-gray-400 font-mono">{p.sku}</p></div>,
      exportValue: p => locale === 'bn' ? p.name_bn : p.name_en,
    },
    {
      header: locale === 'bn' ? 'উপাদান' : 'Items',
      accessor: p => <span className="text-sm text-gray-600">{p.package_items?.length ?? 0} {locale === 'bn' ? 'পণ্য' : 'products'}</span>,
      exportValue: p => p.package_items?.length ?? 0,
    },
    {
      header: locale === 'bn' ? 'ছাড়' : 'Discount',
      accessor: p => <span className={`text-sm font-bold ${p.discount_type !== 'NONE' ? 'text-green-600' : 'text-gray-400'}`}>{discountLabel(p)}</span>,
    },
    {
      header: locale === 'bn' ? 'মূল্য' : 'Price',
      accessor: p => <span className="font-bold text-amber-600">{formatAmount(p.unit_price, locale, 0)}</span>,
      exportValue: p => p.unit_price,
    },
    {
      header: locale === 'bn' ? 'স্টক' : 'Stock',
      accessor: p => <Badge className="font-bold" variant={Number(p.stock_on_hand) > 0 ? 'green' : 'red'}>{formatNumber(Math.round(Number(p.stock_on_hand)), locale)}</Badge>,
    },
    {
  ...
  return (
    <div>
      <PageHeader
        ...
        description={locale === 'bn' ? 'একাধিক পণ্য নিয়ে তৈরি প্যাকেজ পরিচালনা করুন' : 'Manage bundles of multiple products'}
      ...
      <ReusableTable data={data?.data ?? []} columns={columns} keyExtractor={p => p.id}
        ...
        exportFilename="packages" emptyMessage={locale === 'bn' ? 'কোনো প্যাকেজ নেই' : 'No packages yet'}
        quickActions={[{
          label: t('common.edit'),
          render: p => (
