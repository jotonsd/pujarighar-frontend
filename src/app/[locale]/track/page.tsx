...
export default function TrackOrderPage() {
  ...
  const [query, setQuery]             = useState<{ order_number: string; phone: string } | null>(null)

  const { data: order, isLoading, isError } = useTrackByOrderNumberQuery(
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    setQuery({ order_number: orderNumber.trim().toUpperCase(), phone: phone.trim() })
  }
        ...
        <form onSubmit={handleSearch} className="space-y-3">
          ...
          <button type="submit" disabled={isLoading} className="btn-primary w-full">
            {isLoading
              ...
              : (isBn ? 'অর্ডার খুঁজুন' : 'Search Order')}
          </button>
        </form>
      </div>
        ...
        <div className="card space-y-5">
          ...
          <div className="border-t border-gray-100 pt-4 space-y-1">
            ...
            {order.shipping_district && (
              <p className="text-sm text-gray-400">
                {order.shipping_district}{order.shipping_thana ? `, ${order.shipping_thana}` : ''}
              </p>
            )}
          </div>
          ...
          <div className="border-t border-gray-100 pt-4 flex items-center justify-between">
            <div className="text-sm text-gray-500">
              <span className="font-medium text-gray-700">
                {isBn ? order.payment_method_label_bn : order.payment_method_label_en}
              </span>
              {' · '}
          ...
          {order.timeline.length > 0 && (
            <div className="border-t border-gray-100 pt-4">
              ...
              <StatusTimeline logs={order.timeline} locale={locale} deliveryInfo={order.delivery_info} />
            </div>
          )}
        </div>
      )}
