    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="card space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <FloatingInput label={t('profile.fullNameBn')} value={form.full_name_bn} onChange={f('full_name_bn')} />
          <FloatingInput label={t('profile.fullNameEn')} value={form.full_name_en} onChange={f('full_name_en')} />
        </div>
        <div className="grid grid-cols-2 gap-4">
          <FloatingTextarea label={`${t('profile.address')} (বাংলা)`} value={form.address_bn} onChange={f('address_bn')} rows={2} />
          <FloatingTextarea label={`${t('profile.address')} (English)`} value={form.address_en} onChange={f('address_en')} rows={2} />
        </div>
        <div className="grid grid-cols-3 gap-4">
          <FloatingInput label={t('profile.district')} value={form.district} onChange={f('district')} />
          <FloatingInput label={t('profile.thana')} value={form.thana} onChange={f('thana')} />
          <FloatingInput label={t('profile.postCode')} value={form.post_code} onChange={f('post_code')} />
        </div>
