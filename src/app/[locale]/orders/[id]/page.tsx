...
import { useGetOrderQuery, useGetOrderStatusLogQuery, useCancelOrderMutation } from '@/api/orders/ordersApi'
...
export default function OrderDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <button onClick={() => router.back()} className="text-amber-600 hover:underline mb-6 text-sm">← {t('common.back')}</button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-4">
                ...
                <div key={item.id} className="space-y-1.5">
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-700 flex items-center gap-1.5">
                      {item.is_package && <span className="text-xs bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-full font-medium">🎁</span>}
                    ...
                    <div className="ml-4 space-y-1">
                      {item.package_items.map((pi, i) => (
                        <div key={i} className="flex items-center gap-2 text-xs text-gray-500">
                          ...
                          <span className="text-gray-400">×{formatNumber(Math.round(parseFloat(pi.quantity)), locale)}</span>
                        </div>
                      ))}
                    </div>
                  )}
          ...
          <div className="card">
            <h2 className="font-semibold text-gray-700 mb-3">{t('order.shipping')}</h2>
            <div className="text-sm text-gray-600 space-y-1">
              <p>{locale === 'bn' ? order.shipping_name_bn : order.shipping_name_en}</p>
              <p>{order.shipping_phone}</p>
              <p>{locale === 'bn' ? order.shipping_address_bn : order.shipping_address_en}</p>
              {order.shipping_district && <p>{order.shipping_district}, {order.shipping_thana} - {order.shipping_post_code}</p>}
            </div>
          </div>
        ...
        <div className="card h-fit">
          <h2 className="font-semibold text-gray-700 mb-4">{t('order.tracking')}</h2>
          <StatusTimeline
            logs={logs}
            locale={locale}
          ...
          <p className="text-xs text-gray-400 text-center mt-2">
            <a href={`/${locale}/orders/${order.id}/tracking`} className="text-amber-600 hover:underline">
