        ...
        <div className="space-y-3">
          {product.images.length > 0 ? product.images.map((img) => (
            <div key={img.id} className="aspect-square bg-gray-100 rounded-xl overflow-hidden">
              ...
              <img src={img.image} alt={locale === 'bn' ? img.alt_bn : img.alt_en} className="w-full h-full object-cover" />
            </div>
          )) : (
            <div className="aspect-square bg-amber-50 rounded-xl flex items-center justify-center text-6xl">🪔</div>
          )}
        </div>
        <div>
          ...
          <div className="flex items-center gap-3 mb-4">
            <span className="text-3xl font-bold text-amber-600">{formatAmount(product.unit_price, locale, 0)}</span>
            {inStock ? <Badge variant="green">{t('product.inStock')}</Badge> : <Badge variant="red">{t('product.outOfStock')}</Badge>}
          </div>
