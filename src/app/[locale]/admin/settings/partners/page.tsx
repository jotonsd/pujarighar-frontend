...
function PaymentHistoryModal({
  ...
  const openEdit = (p: PartnerProfitPayment) => {
    ...
    setForm({
      ...
      paid_date: p.paid_date ?? TODAY_STR,
      note: p.note,
    });
    setShowForm(true);
  };
    ...
    try {
      if (editingPayment) {
        ...
        if (!plData || fetchingPL) {
          toast.error(
            isBn ? "লাভ-ক্ষতির তথ্য অপেক্ষা করুন" : "Please wait for P&L data",
          );
          return;
        }
        await createPayment({
          ...
          <div className="px-6 py-5 bg-amber-50 border-b shrink-0">
              ...
              <>
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                  <FloatingSelect
                    ...
                    onChange={v =>
                      setForm(p => ({ ...p, year: parseInt(v) || p.year }))
                    }
                    options={yearOptions}
                    searchable={false}
                  ...
                  <FloatingSelect
                    ...
                    onChange={v =>
                      setForm(p => ({ ...p, month: parseInt(v) || p.month }))
                    }
                      ...
                      <div>
                        ...
                        {netProfit < 0 && (
                          <p className="text-xs text-red-500 mt-0.5">
                            {isBn
                              ? "⚠ এই মাসে ক্ষতি হয়েছে"
                              : "⚠ Loss recorded this month"}
                          </p>
                        )}
                      </div>
            ...
            <div className="flex gap-2 mt-4">
              <button
                onClick={handleSave}
                disabled={
                  creating || updating || (!editingPayment && fetchingPL)
                }
...
export default function PartnersPage() {
  ...
  const totalEquity = partners.reduce(
    (s, p) => s + parseFloat(p.equity_percentage),
    0,
  );
  const totalInvested = partners.reduce(
    (s, p) => s + parseFloat(p.invested_amount || "0"),
    0,
  );
  const totalOutstanding = partners.reduce(
    (s, p) => s + parseFloat(p.total_balance || "0"),
    0,
  );
      ...
      {partners.length > 0 && (
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6">
          ...
          >
            ...
            <p
              className={`text-2xl font-bold ${Math.abs(totalEquity - 100) < 0.01 ? "text-red-500" : "text-green-600"}`}
            >
              {formatNumber((100 - totalEquity).toFixed(2), locale)}%
            </p>
          </div>
          <div
