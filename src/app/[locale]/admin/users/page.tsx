...
export default function UsersPage() {
  ...
  const toggle = (id: string, isActive: boolean) =>
    isActive ? deactivate(id) : activate(id)
  ...
  const columns: Column<User>[] = [
    ...
    },
    {
      ...
      exportValue: (u) => u.phone,
    },
    {
      header: 'Role',
      accessor: (u) => <Badge variant={roleVariants[u.role]}>{t(`role.${u.role}`)}</Badge>,
    ...
    },
    {
      ...
      accessor: (u) => (
        <ToggleSwitch
          checked={u.is_active}
          onChange={() => toggle(u.id, u.is_active)}
  ...
  return (
    <div>
      ...
      <div className="flex gap-3 mb-4 flex-wrap">
        <div className="w-64">
          <FloatingInput label={t('common.search')} value={search} onChange={(e) => { setSearch(e.target.value); setPage(1) }} />
        </div>
        <div className="w-48">
      ...
      <ReusableTable
        ...
        emptyMessage={locale === 'bn' ? 'কোনো ব্যবহারকারী নেই' : 'No users found'}
        quickActions={[
          {
            label: t('common.edit'),
            render: (u) => (
