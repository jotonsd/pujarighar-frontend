  ...
  const columns: Column<Product>[] = [
    {
      ...
      accessor: (p) => (
        <div>
          <p className="font-medium text-gray-800">{locale === 'bn' ? p.name_bn : p.name_en}</p>
    ...
    },
    {
      ...
      accessor: (p) => (
        <span className="text-sm text-gray-600">{p.package_items?.length ?? 0} {locale === 'bn' ? 'পণ্য' : 'products'}</span>
      ),
    ...
    },
    {
      ...
      accessor: (p) => (
        <span className={`text-sm font-bold ${p.discount_type !== 'NONE' ? 'text-green-600' : 'text-gray-400'}`}>
          {discountLabel(p)}
        </span>
      ),
    },
    ...
    },
    {
      ...
      accessor: (p) => (
        <Badge variant={Number(p.stock_on_hand) > 0 ? 'green' : 'red'}>{Math.round(Number(p.stock_on_hand))}</Badge>
      ),
    },
    {
