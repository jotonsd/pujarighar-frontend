...
export default function LedgerPage() {
  ...
  const { data: ledger, isLoading } = useGetLedgerQuery({ accountId, from, to }, { skip: !accountId })
  ...
  const columns: Column<LedgerLine>[] = [
    ...
    {
      header: t('debit'),
      accessor: (l) => <span className="text-right block font-bold text-gray-800">{Number(l.debit) ? formatAmount(l.debit, locale, 0) : <span className="text-gray-300 font-normal">—</span>}</span>,
      headerClassName: 'px-4 py-3 text-right text-xs font-semibold text-amber-600 uppercase tracking-wider',
      exportValue: (l) => Number(l.debit) || 0,
    },
    {
      header: t('credit'),
      accessor: (l) => <span className="text-right block font-bold text-gray-800">{Number(l.credit) ? formatAmount(l.credit, locale, 0) : <span className="text-gray-300 font-normal">—</span>}</span>,
      headerClassName: 'px-4 py-3 text-right text-xs font-semibold text-amber-600 uppercase tracking-wider',
      exportValue: (l) => Number(l.credit) || 0,
    },
    {
      header: t('balance'),
      accessor: (l) => <span className="text-right block font-bold text-amber-600">{formatAmount(l.balance, locale, 0)}</span>,
      headerClassName: 'px-4 py-3 text-right text-xs font-semibold text-amber-600 uppercase tracking-wider',
    ...
    <div>
      ...
      {ledger && (
        <div className="flex justify-between text-sm text-gray-600 mb-3 px-1">
          <span>{locale === 'bn' ? 'প্রারম্ভিক ব্যালেন্স' : 'Opening Balance'}: <strong>{formatAmount(ledger.opening_balance, locale)}</strong></span>
          <span>{locale === 'bn' ? 'সমাপ্তি ব্যালেন্স' : 'Closing Balance'}: <strong className="text-amber-600">{formatAmount(ledger.closing_balance, locale)}</strong></span>
        </div>
      )}
      ...
      <ReusableTable
        ...
        emptyMessage={accountId ? (locale === 'bn' ? 'কোনো এন্ট্রি নেই' : 'No entries found') : (locale === 'bn' ? 'একটি অ্যাকাউন্ট বেছে নিন' : 'Select an account to view ledger')}
        skeletonRows={6}
      />
    </div>
  )
