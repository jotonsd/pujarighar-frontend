...
const EMPTY_LINE = (): ManualJournalLine => ({ account_code: '', debit: '', credit: '', memo_bn: '' })

// ─── Manual Entry Modal ───────────────────────────────────────────────────────
function ManualEntryModal({ onClose, isBn }: { onClose: () => void; isBn: boolean }) {
  const { data: accounts = [] } = useGetAccountsQuery()
  const [createJournal, { isLoading }] = useCreateManualJournalMutation()

  const [refType, setRefType] = useState<'EXPENSE' | 'EQUITY' | 'ADJUSTMENT' | 'PURCHASE'>('EXPENSE')
  const [descBn, setDescBn] = useState('')
  const [descEn, setDescEn] = useState('')
  const [lines, setLines] = useState<ManualJournalLine[]>([EMPTY_LINE(), EMPTY_LINE()])

  const totalDebit  = lines.reduce((s, l) => s + (parseFloat(l.debit  || '0') || 0), 0)
  const totalCredit = lines.reduce((s, l) => s + (parseFloat(l.credit || '0') || 0), 0)
  ...
  const handleSubmit = async () => {
    if (!descBn.trim()) { toast.error(isBn ? 'বিবরণ আবশ্যক' : 'Description required'); return }
    const validLines = lines.filter(l => l.account_code && (parseFloat(l.debit || '0') > 0 || parseFloat(l.credit || '0') > 0))
    if (validLines.length < 2) { toast.error(isBn ? 'কমপক্ষে ২টি লাইন প্রয়োজন' : 'At least 2 lines required'); return }
    if (!balanced) { toast.error(isBn ? 'ডেবিট ও ক্রেডিট সমান হতে হবে' : 'Debits must equal credits'); return }
    try {
      await createJournal({ description_bn: descBn, description_en: descEn, reference_type: refType, lines: validLines }).unwrap()
      toast.success(isBn ? 'জার্নাল এন্ট্রি তৈরি হয়েছে' : 'Journal entry created')
      onClose()
  ...
  const typeOptions = REF_TYPES.map(r => ({ value: r.value, label: isBn ? r.label_bn : r.label_en }))
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-3xl max-h-[92vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b shrink-0">
          <h2 className="font-bold text-gray-800">{isBn ? 'নতুন জার্নাল এন্ট্রি' : 'New Journal Entry'}</h2>
        ...
        <div className="overflow-auto flex-1 px-6 py-4 space-y-4">
          ...
          <div>
            <div className="space-y-2">
              {lines.map((line, i) => (
                <div key={i} className="grid grid-cols-[2fr_110px_110px_1fr_auto] gap-2 items-center">
                  <FloatingSelect
                    label={isBn ? 'অ্যাকাউন্ট' : 'Account'}
                    value={line.account_code}
                    onChange={v => updateLine(i, 'account_code', v)}
                    options={acctOptions}
                    searchPlaceholder={isBn ? 'কোড বা নাম...' : 'Code or name...'}
                    showClearButton={false}
                    dropdownZIndex={1010}
                  />
                  <FloatingInput
            ...
            >
              <Plus className="w-3.5 h-3.5" /> {isBn ? 'লাইন যোগ করুন' : 'Add line'}
            </button>
          </div>

          {/* Totals */}
          <div className={`flex justify-end gap-8 px-4 py-3 rounded-xl text-sm font-semibold ${balanced ? 'bg-green-50 border border-green-200' : 'bg-amber-50 border border-amber-200'}`}>
            <div className="text-center">
              <p className="text-xs text-gray-500 mb-0.5">{isBn ? 'মোট ডেবিট' : 'Total Debit'}</p>
              <p className="text-gray-800">{formatAmount(String(totalDebit.toFixed(2)), 'en')}</p>
            </div>
            <div className="text-center">
              <p className="text-xs text-gray-500 mb-0.5">{isBn ? 'মোট ক্রেডিট' : 'Total Credit'}</p>
              <p className="text-gray-800">{formatAmount(String(totalCredit.toFixed(2)), 'en')}</p>
            </div>
            <div className="text-center">
              <p className="text-xs text-gray-500 mb-0.5">{isBn ? 'পার্থক্য' : 'Difference'}</p>
              <p className={balanced ? 'text-green-600' : 'text-amber-600'}>
                {balanced ? (isBn ? '✓ সুষম' : '✓ Balanced') : formatAmount(String(Math.abs(totalDebit - totalCredit).toFixed(2)), 'en')}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-3 px-6 py-4 border-t shrink-0">
          <button onClick={handleSubmit} disabled={isLoading || !balanced} className="flex-1 btn-primary disabled:opacity-50">
            {isLoading ? (isBn ? 'সংরক্ষণ হচ্ছে...' : 'Saving...') : (isBn ? 'জমা করুন' : 'Post Entry')}
          </button>
          <button onClick={onClose} className="flex-1 btn-secondary">{isBn ? 'বাতিল' : 'Cancel'}</button>
        </div>
      </div>
    </div>
  )
...
export default function JournalPage() {
  ...
  return (
    <div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {entries.map((entry, ei) => {
                const badge = REF_BADGE[entry.reference_type] ?? { variant: 'gray', label_bn: entry.reference_type, label_en: entry.reference_type }
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-2 text-gray-800 font-medium align-top max-w-[160px] truncate">
                        {isFirst ? (isBn ? entry.description_bn : entry.description_en) : ''}
                      </td>
                      ...
                      <td className="px-4 py-2 text-xs text-gray-500 align-top whitespace-nowrap">
                        {isFirst ? new Date(entry.created_at).toLocaleDateString(isBn ? 'bn-BD' : 'en-US') : ''}
                      </td>
                    </tr>
                  )
                })
          ...
          {totalPages > 1 && (
            <div className="px-4 py-3 border-t border-gray-100 flex items-center justify-between">
              <span className="text-xs text-gray-500">
                {isBn ? `পেজ ${page} / ${totalPages}` : `Page ${page} of ${totalPages}`}
              </span>
      ...
      {showForm && <ManualEntryModal onClose={() => setShowForm(false)} isBn={isBn} />}
    </div>
  )
}
